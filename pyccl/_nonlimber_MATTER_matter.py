...
def _choose_returned_cl(cls_matter, clt1, clt2, noni_1, noni_2):
    parse_str = 'dd'
    if noni_1 == noni_2 and True not in noni_1:
        parse_str = 'll'
    if noni_1 != noni_2:
        parse_str = 'dl'
    ...
    for key in cls_matter[parse_str].keys():
        ...
        #print(key, cls_matter[parse_str][key][0])
...
def _get_chi_arrs(cosmo, clt, Nchi_nonintegrated=800, Nchi_integrated=1600):
    ...
    for i in range(len((t))):
        ...
        chi_max_noni = max(chi_max_noni, chis_t[i][len(mask) - np.argmax(mask[::-1])-1])
        chi_max_i = max(chi_max_i, chis_t[i][len(mask) - np.argmax(mask[::-1])-1])

        if bessels_t[i] >= 0:
            nonintegrated.append(True)
            
            chi_min_noni = min(chi_min_noni, chis_t[i][np.argmax(mask)])
        if bessels_t[i] == -1:
            nonintegrated.append(False)
            chi_min_i = 1.e-8


    #print(chi_min_i, chi_max_i, chi_min_noni, chi_max_noni)
    if chi_min_i== np.inf:
        chi_min_i = 1.e-8
    if chi_min_noni==np.inf:
        chi_min_noni=1.e-8
    chi_noni = np.linspace(chi_min_noni, chi_max_noni, num=Nchi_nonintegrated)
    chi_i = np.linspace(chi_min_i, chi_max_i, num=Nchi_integrated)
    kerfac_noni = np.zeros(Nchi_nonintegrated)
    kerfac_i = np.zeros(Nchi_integrated)
    #print(kerfac_i.shape, kerfac_noni.shape)
    #print(bessels_t)

    for i in range(len(nonintegrated)):
        if nonintegrated[i]:
            kern = clt.get_kernel(chi_noni)[0]
            #print(kern)

            trans = clt.get_transfer(0.,
                                     ccl.scale_factor_of_chi(cosmo, chi_noni))[0]
            kerfac_noni += kern * trans            

        else:
            kern = clt.get_kernel(chi_i)[0]
            #print(kern)
            trans = clt.get_transfer(0.,
                                     ccl.scale_factor_of_chi(cosmo, chi_i))[0]            
            kerfac_i[1:] += kern[1:] * trans[1:] / chi_i[1:]**2
    '''
    ...
    return nonintegrated, [chi_noni], [chi_i], [kerfac_noni], [kerfac_i]
...
def _nonlimber_MATTER(
    ...
    #print(fll_t1, fll_t2)
    #print((noni_1, chi_noni_1, chi_i_1, kerfac_noni_1, kerfac_i_1))
    ...
    '''
    if False in noni_1 or False in noni_2:
        Nk_fft *= 2
    if True in noni_1 or True in noni_2:
        st *= 4
        st_spline *= 2
    ...
    if type(lmax) is not str:
        ...
        for el in range(100, int(ls[-1])+1, int(ls[-1])-100):
            lmax = el
            ma.set((chi_nonintegrated, chi_integrated),
                   (kerfac_noni, kerfac_i), (a_pk, chi_pk, k_pk, deltaksq), growth,
                   lmax=lmax, uses_separability=seperability,
