...
def halomod_trispectrum_2h_22_linear_bias(cosmo, hmc, k, a, prof, *, 
                              bias1=1, bias2=1, bias3=1, bias4=1,
                              p_of_k_a=None):
    """ Computes the "22" term of the isotropized halo model 2-halo trispectrum
    for four profiles :math:`u_{1,2}`, :math:`v_{1,2}` as
    ...
    out = np.zeros([na, nk, nk])
    for ia, aa in enumerate(a_use):
        norm = prof.get_normalization(cosmo, aa, hmc=hmc)**4
        ...
        iii = hmc.I_1_2(cosmo, k_use, aa, prof,
                        prof_2pt=prof_2pt, diag=False)
        ...
        tk_2h_22 = p * (i13 * i24 + i14 * i32) * iii
        # Normalize
        out[ia, :, :] = tk_2h_22 / norm
...
def halomod_trispectrum_2h_13_linear_bias(cosmo, hmc, k, a, prof, *,
    ...
    for ia, aa in enumerate(a_use):
        # Compute profile normalizations
        norm = prof.get_normalization(cosmo, aa, hmc=hmc)**3
        #norm1, norm2, norm3, norm4 = _get_norms(prof, prof2, prof3, prof4,
        #                                        cosmo, aa, hmc)
        #norm = norm1 * norm2 * norm3 * norm4
        ...
        # Compute trispectrum at this redshift
        p1 = pk2d(k_use, aa, cosmo)[None, :]
        i1 = bias1#hmc.I_1_1(cosmo, k_use, aa, prof)[None, :]
        iiii = hmc.I_1_3(cosmo, k_use, aa, prof,
                         prof_2pt=prof_2pt)
        iiii *= 4 * bias1 * bias2 * bias3 * bias4
...
def halomod_trispectrum_3h_linear_bias(cosmo, hmc, k, a, prof, *,
    ...
    for ia, aa in enumerate(a_use):
        ...
        Bpt = get_Bpt(aa)
        tk_3h = Bpt * 4 * bias1 * bias2 * bias3 * bias4 * (iii)
...
def halomod_trispectrum_4h_linear_bias(cosmo, hmc, k, a, prof, bias1=1, bias2=1, bias3=1, bias4=1, p_of_k_a=None):
    """ Computes the isotropized halo model 4-halo trispectrum for four
    profiles :math:`u_{1,2}`, :math:`v_{1,2}` as
    ...
    out = np.zeros([na, nk, nk])
    for ia, aa in enumerate(a_use):
        # no normalizations needed for linear bias expansion of 4h term
        #norm1, norm2, norm3, norm4 = _get_norms(prof, prof2, prof3, prof4,
        #                                        cosmo, aa, hmc)
        #norm = norm1 * norm2 * norm3 * norm4
        ...
        tk_4h = bias1 * bias2 * bias3 * bias4 * (t1113 + t1122)
...
def halomod_Tk3D_cNG_linear_bias(cosmo, hmc, prof,
    ...
    # no approximation can be made here
    tkk = halomod_trispectrum_1h(cosmo, hmc, k_use, a_arr, prof)

    # approximation CAN be made here
    tkk += halomod_trispectrum_2h_22_linear_bias(cosmo, hmc, k_use, a_arr,
                                     prof,
    ...
    # approximation CAN be made here
    tkk += halomod_trispectrum_4h_linear_bias(cosmo, hmc, k_use, a_arr,
                                  prof=prof,
                                  bias1, bias2, bias3, bias4,
                                  p_of_k_a=None)
